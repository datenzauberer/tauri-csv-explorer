...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![get_cli_filename, tauri_read_csv_file])
        ...
        .run(|app, event| {
           ...
           // MacOS specific event listener for when the app is opened with a filename
            if let tauri::RunEvent::Opened { urls } = event {
                ...
                let filename = urls.first().unwrap().to_file_path().unwrap().as_path().display().to_string();
